...
static int alloc_open(struct tcmu_device *dev, bool reopen)
{
    ...
    printf(">>> [ALLOC] dev size: %lli\n", tcmu_get_dev_size(dev));

    state->buf = malloc(sizeof(char) * tcmu_get_dev_size(dev)); // just for a while
    state->size = tcmu_get_dev_size(dev);
    state->ptr = 0;
